        ...
        <tbody>
            ...
            <tr> 
                ...
                @foreach ($refund_status as $status)
                    ...
                    @endphp
                    <td class="text-center"><a href="{{ url('consumers/refunds') }}?{{ http_build_query(['refund_status' => [$status->id],'geo_area'=> [$ga->id]]) }}" target="_blank">{{ $refund_data[$ga->id][$status->id]['count'] ?? 0 }}</a></td>
                @endforeach
            ...
            <tr>
                ...
                <td class="text-end"><a href="{{ url('customers/refunds') }}" target="_blank">{{ numberFormat($tot_amt, 2) }}</a></td>
            </tr>
        </tbody>
    </table>
</div>