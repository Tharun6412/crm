...
@section('page-content')
    ...
    <div>
        <form action="{{ url('calls/closeOtps') }}" id="consumer-otp-search-form" method="GET">
            <div class="rounded bg-info-subtle p-3 mb-3">
                <div class="row justify-content-sm-center">
                    <div class="col-sm-6">
                        <h3 class="text-start">Mobile Number</h3>
                        <div class="position-relative">
                            {{-- Quick search input --}}
                            <div class="input-group input-group-lg">
                                <label for="quickSearch" class="input-group-text bg-white"><i class="bi bi-search"></i></label>
                                <input type="text" name="search" id="search" class="form-control no-focus-ring border-start-0 border-end-0" placeholder="Enter Mobile Number" value="{{ request()->search }}">
        ...
        <div id="consumer-otp-list" class="current-page-reload">
        </div>
    </div>
@endsection
{{-- Scripts --}}
@push('scripts')
    @include('scripts.ajax-form-search', ['form' => 'consumer-otp'])
@endpush