...
<div>
    <dl>
        <dt>Employee ID</dt>
        <dd>{{ $user_source->emp_id }}</dd>
        <dt>Name</dt>
        <dd>{{ $user_source->first_name . ' ' . $user_source->last_name }}</dd>
        <dt>Mobile</dt>
        <dd>{{ $user_source->mobile }}</dd>
        <dt>E-Mail</dt>
        <dd>{{ $user_source->email }}</dd>
    </dl>
    <form action="{{ url('validateRegisterOtp/' . $user_source->id) }}" id="reg_self_otp_form">
        @csrf
        <input type="hidden" name="id" value="{{ $user_source->id }}">
        <div class="row mb-3">
