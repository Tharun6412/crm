                ...
                <form action="{{ url('master/consumer/schemes/'.$scheme->id) }}" id="edit-scheme-form" name="edit-scheme-form">
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-sm-8">
                            ...
                            @foreach ($gas as $ga)
                                @php
                                    $ga_checked = in_array($ga->id, $ga_selected) ? true : false;
                                @endphp
                                <label class="me-3">
                                    <input type="checkbox" name="applicable_ga[]" value="{{ $ga->id }}" @checked($ga_checked)>
                                    {{ $ga->name }}
                                </label>
                            @endforeach
                        </div>
                    </div>
                    <div class="mb-3" id="edit-scheme-error"></div>
                    <button type="submit" class="btn btn-success"><i class="bi bi-pencil-square"></i>&nbsp;Update Scheme</button>
                </form>
            </div>
        </div>
