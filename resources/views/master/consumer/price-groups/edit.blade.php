...
<div class="modal-dialog modal-lg">
    <div class="modal-content">
        <div class="modal-header">
            <h1 class="modal-title fs-5" id="exampleModalLabel">Edit Price Group - {{ $price_group->code }}</h1>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
            <div id="price-group-edit-success">
                <form action="{{ url('master/consumer/price-groups/' . $price_group->id) }}" method="POST" id="price-group-edit-form">
                    @csrf
                    @method('PUT')
                    <div class="row mb-2">
                        <label for="code" class="col-sm-2 col-form-label text-end">Code</label>
                        <div class="col-sm-9">
                            <input type="text" name="code" id="code" class="form-control" value="{{ $price_group->code }}">
                        </div>
                    </div>
                    <div class="row mb-2">
                        <label for="description" class="col-sm-2 col-form-label text-end">Description</label>
                        <div class="col-sm-9">
                            <input type="text" name="description" id="description" class="form-control" value="{{ $price_group->description }}">
                        </div>
                    </div>
                    <div class="row mb-2">
                        ...
                        <div class="col-sm-9">
                            <select name="segment_id" id="segment_id" class="form-select">
                                ...
                                @foreach ($segments as $item)
                                    <option value="{{ $item->id }}" @selected($item->id == $price_group->segment_id)>{{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-9">
                            <select name="ga_id" id="ga_id" class="form-select">
                                ...
                                @foreach ($geo_areas as $item)
                                    <option value="{{ $item->id }}" @selected($item->id == $price_group->ga_id)>{{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-9">
                            <div class="row mb-2">
                                <div class="col-sm-4">
                                    ...
                                    <input type="text" name="basic" class="form-control text-end" id="basic" value="{{ $price_group->basic }}">
                                </div>
                                <div class="col-sm-4">
                                    <label for="vat" class="col-form-label">VAT(%)</label>
                                    <input type="text" name="vat" id="vat" class="form-control text-end" value="{{ $price_group->vat }}">
                                </div>
                                <div class="col-sm-4">
                    ...
                    <div class="row mb-2">
                        ...
                        <div class="col-sm-9">
                            <div class="input-group">
                                <input type="text" name="effective_from" id="effective_from" class="form-control" placeholder="DD-MM-YYYY" value="{{ $price_group->effective_from?->format('d-m-Y') }}">
                                <label for="effective_from" class="input-group-text"><i class="bi bi-calendar3"></i></label>
                            </div>
                        </div>
                    </div>
                    <div id="price-group-edit-error" class="text-danger"></div>
                    <div class="row">
...
{{-- Scripts --}}
@include('scripts.ajax-form-submit', ['form' => 'price-group-edit'])
