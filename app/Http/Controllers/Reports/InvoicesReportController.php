...
class InvoicesReportController extends Controller
{
    ...
    {
       /* if($request->ajax()|| (!empty($request->date_from)) and (!empty($request->date_to))) {
            // Validation
        ...
        if (
            ...
            $query = BillInvoice::query()
                ...
                ->when($request->filled('invoice_type'), fn ($q) => $q->whereIn('type_id', (array) $request->invoice_type))
                ->when($request->filled('connection_type_id'), fn ($q) => $q->whereIn('prepaid', (array) $request->connection_type_id))
                ->when(
                    $request->filled('segments') ||
                    $request->filled('geo_area'),
                    function ($q) use ($request) {
            ...
            if ($request->ajax()) {
                return view(
                    'reports.invoice.invoice-report.list-body',
                    compact(
                        'invoices',
                        'tRecords',
                        'pageTotals',
            ...
            return view(
                'reports.invoice.invoice-report.list',
                compact(
                    'invoices',
                    'tRecords',
                    'pageTotals',
     ...
     * Inovices List
     */
    public function listCounts(Request $request)
    {
        // Query
        $query = BillInvoice::query()
            ->when($request->filled('key'), fn ($q) => $q->where(fn ($q) =>
        ...
        return view('reports.invoice.invoice-report.invoices-list-counts', compact('totals'));
    }
}