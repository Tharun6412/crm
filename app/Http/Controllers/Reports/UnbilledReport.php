<?php
namespace App\Http\Controllers\Reports;

use App\Enums\ConnectionType;
use App\Enums\ConsumerStatus;
use App\Enums\InvoiceStatus;
use App\Enums\InvoiceType;
use App\Exports\Consumers\ConsumerExport;
use App\Exports\Reports\UnbilledConsumersExport;
use App\Http\Controllers\Controller;
use App\Models\Consumer\Consumer;
use App\Models\Consumer\ConsumerStatus as ConsumerConsumerStatus;
use App\Models\Invoice\BillInvoice;
use App\Models\Master\Ga;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class UnbilledReport extends Controller
{
    public function index(Request $request)
    {
        $gas = Ga::select('id', 'name')->get();

        $consumers = DB::table('cns_consumers')
            ->where('cns_consumers.status_id', ConsumerStatus::ACTIVATE->value)
            ->where('cns_consumers.connection_type_id', ConnectionType::POSTPAID->value)
            ->when(
                $request->filled('segments'),
                fn($q) => $q->whereIn('cns_consumers.segment_id', $request->segments)
            )
            ->select([
                'cns_consumers.ga_id',
                // #1 Total active consumers
                DB::raw('COUNT(cns_consumers.id) as total_active'),
                // #2 Eligible (activated > 60 days ago)
                DB::raw('COUNT(CASE WHEN cns_consumers.activation_Date <= \''. now()->subDays(60)->toDateTimeString() .'\' THEN 1 END) as eligible_count'),

                // #3 Unbilled 60-90 days
                DB::raw('COUNT(CASE WHEN
                    (
                        (cns_consumers.last_invoice_date IS NOT NULL
                            AND cns_consumers.last_invoice_date < \''. now()->subDays(60)->toDateTimeString() .'\'
                            AND cns_consumers.last_invoice_date >= \''. now()->subDays(90)->toDateTimeString() .'\')
                        OR
                        (cns_consumers.last_invoice_date IS NULL
                            AND cns_consumers.activation_date < \''. now()->subDays(60)->toDateTimeString() .'\'
                            AND cns_consumers.activation_date >= \''. now()->subDays(90)->toDateTimeString() .'\')
                    )
                THEN 1 END) as unbilled_60_90'),

                // #4 Unbilled 90-120 days
                DB::raw('COUNT(CASE WHEN
                    (
                        (cns_consumers.last_invoice_date IS NOT NULL
                            AND cns_consumers.last_invoice_date < \''. now()->subDays(90)->toDateTimeString() .'\'
                            AND cns_consumers.last_invoice_date >= \''. now()->subDays(120)->toDateTimeString() .'\')
                        OR
                        (cns_consumers.last_invoice_date IS NULL
                            AND cns_consumers.activation_date < \''. now()->subDays(90)->toDateTimeString() .'\'
                            AND cns_consumers.activation_date >= \''. now()->subDays(120)->toDateTimeString() .'\')
                    )
                THEN 1 END) as unbilled_90_120'),

                // #5 Unbilled >120 days
                DB::raw('COUNT(CASE WHEN
                    (
                        (cns_consumers.last_invoice_date IS NOT NULL
                            AND cns_consumers.last_invoice_date < \''. now()->subDays(120)->toDateTimeString() .'\')
                        OR
                        (cns_consumers.last_invoice_date IS NULL
                            AND cns_consumers.activation_date < \''. now()->subDays(120)->toDateTimeString() .'\')
                    )
                THEN 1 END) as unbilled_120_plus'),
            ])
            ->groupBy('cns_consumers.ga_id')
            ->orderBy('cns_consumers.ga_id')
            ->get()
            ->keyBy('ga_id');

        // Totals across all GAs
        $totals = (object)[
            'total_active'    => $consumers->sum('total_active'),
            'eligible_count'  => $consumers->sum('eligible_count'),
            'unbilled_60_90'  => $consumers->sum('unbilled_60_90'),
            'unbilled_90_120' => $consumers->sum('unbilled_90_120'),
            'unbilled_120_plus' => $consumers->sum('unbilled_120_plus'),
        ];

        if ($request->ajax()) {
            return view('reports.consumer.unbilled-report.list-body', [
                'gas'       => $gas,
                'consumers' => $consumers,
                'totals'    => $totals,
            ]);
        }

        return view('reports.consumer.unbilled-report.list', [
            'gas'       => $gas,
            'consumers' => $consumers,
            'totals'    => $totals,
        ]);
    }

    public function list(Request $request)
    {
        // Prepare data
        $sortBy = ($request->get('sortBy')) ? $request->get('sortBy') : 'cns_consumers.created_at';
        $sortOr = ($request->get('sortOr')) ? $request->get('sortOr') : 'desc';
        $records = (int) $request->get('records', 20);

        $consumers = Consumer::with([
            'ga:id,name',
            'segment:id,name',
            'connectType:id,name',
            'statusHistory',
            'status:id,name',
            'latestInvoice:id,consumer_id,invoice_date,total_amount,invoice_number,status_id',
            'latestInvoice.status:id,name',
            'latestInvoice.consumption:id,invoice_id,net_consumption'
        ])
        ->whereIn('cns_consumers.status_id',[ConsumerStatus::ACTIVATE->value])
        ->where('cns_consumers.connection_type_id', ConnectionType::POSTPAID->value)
        ->select([
            'cns_consumers.id','cns_consumers.crn', 'cns_consumers.segment_id', 'cns_consumers.fname', 'cns_consumers.ga_id', 'cns_consumers.status_id', 'cns_consumers.connection_type_id'
        ])
        ->when($request->filled('aging'), function ($q) use ($request) {
            match ($request->aging) {
                '60_90'  => $q->where(function ($q) {
                                $q->where(function ($q) {
                                    $q->whereNull('cns_consumers.last_invoice_date')
                                    ->where('cns_consumers.activation_date', '<', now()->subDays(60)->toDateTimeString())
                                    ->where('cns_consumers.activation_date', '>=', now()->subDays(90)->toDateTimeString());
                                })
                                ->orWhere(function ($q) {
                                    $q->whereNotNull('cns_consumers.last_invoice_date')
                                        ->where('cns_consumers.last_invoice_date', '<', now()->subDays(60)->toDateTimeString())
                                        ->where('cns_consumers.last_invoice_date', '>=', now()->subDays(90)->toDateTimeString());
                                });
                            }),

                '90_120' => $q->where(function ($q) {
                                $q->where(function ($q) {
                                    $q->whereNull('cns_consumers.last_invoice_date')
                                    ->where('cns_consumers.activation_date', '<', now()->subDays(90)->toDateTimeString())
                                    ->where('cns_consumers.activation_date', '>=', now()->subDays(120)->toDateTimeString());
                                })
                                ->orWhere(function ($q) {
                                    $q->whereNotNull('cns_consumers.last_invoice_date')
                                        ->where('cns_consumers.last_invoice_date', '<', now()->subDays(90)->toDateTimeString())
                                        ->where('cns_consumers.last_invoice_date', '>=', now()->subDays(120)->toDateTimeString());
                                });
                            }),

                'gt_120' => $q->where(function ($q) {
                                $q->where(function($q) {
                                    $q->whereNull('cns_consumers.last_invoice_date')
                                        ->where('cns_consumers.activation_date', '<', now()->subDays(120)->toDateTimeString());
                                })
                                ->orWhere(function ($q) {
                                    $q->whereNotNull('cns_consumers.last_invoice_date')
                                        ->where('cns_consumers.last_invoice_date', '<', now()->subDays(120)->toDateTimeString());
                                });
                            }),
                default  => null // 'all' — no extra filter
            };
        })
        // ← Move the default 60-day condition here, outside the aging block
        ->when(!$request->filled('aging') || $request->aging === 'all', function ($q) {
            $q->where(function ($q) {
                $q->whereNull('cns_consumers.last_invoice_date')
                ->where('cns_consumers.activation_date', '<=', now()->subDays(60)->toDateTimeString())
                ->orWhere('cns_consumers.last_invoice_date', '<=', now()->subDays(60)->toDateTimeString());
            });
        })
        ->when($request->filled('key'), function ($q) use ($request) {
            $q->where(function ($query) use ($request) {
                $query->whereAny(['crn', 'fname', 'phone'], 'like', '%' . $request->key . '%')
                ->orWhereHas('meter', function ($q1) use ($request) {
                    $q1->whereAny(['meter_no', 'meter_serial_no'], 'like', '%' . $request->key . '%');
                });
            });
        })
        ->when($request->filled('geo_area'), fn($q) => $q->whereIn('cns_consumers.ga_id', $request->geo_area))
        ->when($request->filled('segments'), fn($q) => $q->whereIn('cns_consumers.segment_id', $request->segments))
        ->when($request->filled('connection_type_id'), function ($q) use($request) {
            $q->where('connection_type_id', $request->connection_type_id);
        })
        ->orderBy($sortBy, $sortOr)->paginate($records)->withQueryString();
        
        // dd($consumers);
        if($request->ajax()) {
            return view('reports.consumer.unbilled-report.consumers-list-body', [
                    'consumers' => $consumers
                ]);
        }
        return view('reports.consumer.unbilled-report.consumers-list',[
            'consumers' => $consumers
        ]);
    }

    /**
     * Consumers Export
     */
    public function listExport(Request $request)
    {
        return (new UnbilledConsumersExport($request))->download('unbilled-consumers.xlsx');
    }
}
?>