...
class BillingController extends Controller
{
    ...
    public function storeGasBill(Request $request, $id)
    {
        ...
        if($total_no_days < 1) {
            return response()->json(['message' => 'Billing Frequency should be greater than 1 day.'], 422);
        }
    ...
    {   
        ...
        $avg_price = ($net_consumption > 0) ? ($inv_base_amt / $net_consumption) : $prices->last()->basic_price ;
        $tax_value = $prices->last()->tax_value; // tax percentage.
        $inv_tax_amt =  round((($inv_base_amt * $tax_value) / 100), 2);
        $inv_total =  $inv_base_amt + $inv_tax_amt;
